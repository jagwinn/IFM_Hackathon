...
[`fixtures/routing_tests.json`](src/horizon_relay/fixtures/routing_tests.json) holds 27 labeled requests: 7 for the 0.9B, 8 for the 4B, 7 for the cloud, 2 large goals that must be split up and handed to the local models, and 3 that force a path (0.9B → 4B, cloud plan-and-delegate, cloud direct). Each lists the models allowed to answer and, where possible, a pattern the answer must match or, for the delegation cases, what the run itself must show (`expects`: how many subtasks reached local models, how few stayed on the cloud, and whether two or more ran at the same time). A plan for a large goal that comes back as one task doing everything is rejected, and the planner is asked again with that as the reason (`RoutingPolicy.check_plan`, `bulk_parts`, `min_parallel_local`).
